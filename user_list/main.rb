...
users.map{ |users| User_services.create_user(users[:id], users[:name], users[:email],users[:phone])}
contacts.map{ |contacts| Contact_services.create_contact(contacts[:id], contacts[:name], contacts[:email],contacts[:phone], contacts[:user_id])}
to_json = Response_service.response_as_json(contacts, users)